...
class Reporter:
    ...
    def build_network_section(self, results: Dict[str, NetworkResult]) -> str:
        ...
        for name, res in results.items():
            lines.append(
                f"{name:<20} Ping: {res.average:.1f}ms | "
                f"Pertes: {res.packet_loss:.1f}% | Jitter: {res.jitter:.1f}ms | "
    ...
    def build_performance_section(self, before: Dict[str, float] | None, after: Dict[str, float]) -> str:
        ...
        if before and "latency" in before and "latency" in after:
            delta = after["latency"] - before["latency"]
            pct = (delta / before["latency"]) * 100 if before["latency"] else 0
